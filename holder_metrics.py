...
TOKEN_SELECTION_MODE = os.getenv("TOKEN_SELECTION_MODE", "pump").lower()  # Options: pump, non-pump, all
    ...
    try:
        async with session.post(SOLANA_MAINNET_HTTP, json=request) as response:
            if response.status == 429:
                logger.warning(f"Rate limit hit for {mint_address}, waiting before retry...")
                await asyncio.sleep(2)  # Wait before retrying
                return mint_address, None  # Signal retry needed
            
            if response.status != 200:
                logger.error(f"HTTP {response.status} error for {mint_address}: {await response.text()}")
                return mint_address, None  # Signal retry needed
...
async def get_token_accounts_batch(session, mint_addresses, batch_size=25, max_retries=3):
    ...
    for i in range(0, len(mint_addresses), batch_size):
        ...
        while retry_count < max_retries:
            ...
            batch_tasks = [fetch_single_token(session, mint) for mint in batch if mint not in all_accounts]
            if not batch_tasks:
                break
        ...
        if retry_tokens:
            logger.warning(f"Failed to fetch data for {len(retry_tokens)} tokens after {max_retries} retries")
...
async def update_token_metrics():
    ...
    params = [cutoff_time_str, MIN_SUPPLY]
    ...
    if not tokens_to_check:
        logger.info(f"No recently updated {TOKEN_SELECTION_MODE} tokens found in the last {token_age_minutes:.1f} minutes (min supply: {MIN_SUPPLY:,})")
        conn.close()
        return

    logger.info(f"Found {len(tokens_to_check)} recently updated {TOKEN_SELECTION_MODE} tokens in the last {token_age_minutes:.1f} minutes...")
