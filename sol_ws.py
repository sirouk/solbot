...
dotenv.load_dotenv()
MIN_SUPPLY = os.getenv("MIN_SUPPLY", 10_000)
...
SOLANA_MAINNET_HTTP = os.getenv("SOLANA_MAINNET_HTTP", "https://api.mainnet-beta.solana.com")
DB_FILE = "token_watch.db"
TOKEN_PROGRAM_ID = os.getenv("TOKEN_PROGRAM_ID", "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA")
INITIALIZE_MINT_IX = "Program log: Instruction: InitializeMint"  # Log signature for mint initialization
...
def init_db():
    ...
    # Only keep tokens table with added owner column
...
async def get_token_metadata_from_tx(tx_data: dict) -> dict:
    ...
    for inner in tx["meta"]["innerInstructions"]:
        for ix in inner["instructions"]:
            if (ix.get("program") == "spl-token" and 
                ix.get("parsed", {}).get("type") == "initializeMint2"):
                info = ix["parsed"]["info"]
    ...
    for inner in tx["meta"]["innerInstructions"]:
        for ix in inner["instructions"]:
            if (ix.get("program") == "spl-token" and 
                ix.get("parsed", {}).get("type") == "setAuthority"):
                info = ix["parsed"]["info"]
                if info["authorityType"] == "mintTokens" and info["newAuthority"] is None:
                    token_data["has_mint_authority"] = False
    
    # Calculate actual supply
    if "raw_supply" in token_data and "decimals" in token_data:
        token_data["actual_supply"] = token_data["raw_supply"] / (10 ** token_data["decimals"])
                ...
                try:
                    ...
                    if "params" in data and "result" in data["params"]:
                        ...
                        if is_mint_operation:
                            ...
                            logger.info(f"Found mint initialization! Signature: {signature}")
                            ...
                            if token_data:
                                ...
                                if actual_supply >= MIN_SUPPLY:
                                    ...
                                    logger.info(f"Mint address: {token_data['mint_address']}")
                                    ...
                                    logger.info(f"  - Can mint more: {'🚨 YES' if token_data['has_mint_authority'] else '✅ NO'}")
                                    logger.info(f"  - Can freeze: {'🚨 YES' if token_data['has_freeze_authority'] else '✅ NO'}")
                                    
                                   # Display safety information
                                    if not token_data["has_mint_authority"] and not token_data["has_freeze_authority"]:
                                    ...
                                    logger.info(f"Skipping token with insufficient supply: {actual_supply:,.2f}")
